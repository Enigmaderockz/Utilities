...
def generate_data_chunk(chunk_size):
    chunk_data = [generate_record() for _ in range(chunk_size)]
    return chunk_data

# Function to write data to CSV in chunks
...
# Main function to generate and write the CSV file with parallel execution
def generate_large_csv(total_records, output_file):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for chunk_id in range(num_chunks):
            futures.append(executor.submit(generate_data_chunk, chunk_size))

        # Handle remaining records if any
        if remaining_records > 0:
            futures.append(executor.submit(generate_data_chunk, remaining_records))
