...
# Define a dictionary to store copied files by destination path
copied_files = {destination_path: [] for destination_path in keyword_paths.values()}
...
for filename in os.listdir(directory):
    ...
    for keyword, destination_path in keyword_paths.items():
        if filename.startswith(keyword) and business_date in filename:
            ...
            # Add the copied file to the list for its destination path
            copied_files[destination_path].append(filename)

# Print copied files in groups by destination path
for destination_path, files in copied_files.items():
    print(f"Files copied to path {destination_path} using command:")
